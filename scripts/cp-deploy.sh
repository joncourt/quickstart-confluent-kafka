...
locate_start_scripts() {
    echo "Locating start scripts" >> $LOG
    BIN_DIR=${CP_HOME}/bin
...
configure_confluent_zk() {
    echo "Configuration Confluent Zookeeper" >> $LOG
...
configure_kafka_broker() {
    ...
    set_property $BROKER_CFG "listeners" "PLAINTEXT://$THIS_HOST:9092"
...
#
wait_for_zk_quorum() {
    echo "Waiting for Zookeeper quorum to form" >> $LOG
...
create_topic_safely() {
    ...
    echo "Attempting to create topics, will retry $MAX_TOPIC_RETRIES times at intervals of $RETRY_INTERVAL_SEC seconds" >> $LOG
